...
@pytest.fixture
def service():
    """Fixture to provide a fresh service instance for each test"""
...
def test_get_item():
    ...
    item = Item(name="Test Item", description="Test Description")
    created_item = service.create_item(item)
    retrieved_item = service.get_item(created_item.id)
...
def test_update_item():
    ...
    result = service.update_item(created_item.id, updated_item)
...
def test_delete_item():
    ...
    # Verify item exists
    assert service.get_item(created_item.id) is not None
    ...
    # Verify item is gone
    assert service.get_item(created_item.id) is None
...
def test_multiple_items(service):
    ...
    created_items = [service.create_item(item) for item in items]
